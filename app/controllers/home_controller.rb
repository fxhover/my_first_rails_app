...
class HomeController < ActionController::Base
  ...
  def update
    if request.post?
      @article = Article.find(params[:id])
      if @article.update_attributes({:title=>params[:title],:content=>params[:content]})
  ...
  def new
    if request.post?
       ...
       if (content.slice(0, 7) != 'http://')
         content = 'http://' + content
       end

       @article = Article.new({:title=>params[:title], :content=>content})
       if @article.save
          redirect_to :action=> 'index'
       else
         render action: "new"
  ...
  def comments
    @article = Article.find(params[:id])
    @comments = Comments.where({:article_id => @article.id}).all()
  ...
  def addcomments
    if request.post?
      ...
      if @article.nil?
         ...
         @comment = Comments.new({:article_id => @article.id, :content => params[:content]})
         if @comment.save
